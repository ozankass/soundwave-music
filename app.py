...
try:
    ...
    from config import FRONTEND_DIR, DOWNLOADS_DIR
    from auth import hash_password, verify_password, generate_user_token, verify_user_token
    from database import load_db, save_db
    from radio_service import get_radio_stations
...
# Static Mounts & HTML
# -------------------------------------------------------------
if (FRONTEND_DIR / "css").exists() or (FRONTEND_DIR / "js").exists():
...
def serve_home():
    ...
    if not idx_path.exists():
        idx_path = Path(__file__).resolve().parent / "index.html"
...
def serve_css():
    ...
    for c in candidates:
        if c.exists():
            return FileResponse(c, media_type="text/css")
...
def serve_app_js():
    candidates = [
        ...
        FRONTEND_DIR / "app.js",
        Path(__file__).resolve().parent / "app.js"
    ]
    for c in candidates:
        if c.exists():
...
@app.get("/static/js/auth.js")
@app.get("/auth.js")
def serve_auth_js():
    candidates = [
        FRONTEND_DIR / "js" / "auth.js",
        FRONTEND_DIR / "auth.js",
        Path(__file__).resolve().parent / "auth.js"
    ]
    for c in candidates:
        if c.exists():
            return FileResponse(c, media_type="application/javascript")
